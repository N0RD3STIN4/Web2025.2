...
class View:
    ...
    def horario_inserir(data, confirmado, id_cliente, id_servico, id_profissional):
        for obj in View.horario_listar():
            if (
                obj.get_id() != id
                and obj.get_data() == data
                and obj.get_id_profissional() == id_profissional
            ):
                raise ValueError("Horário já cadastrado para esse profissional")
        c = Horario(0, data)
    ...
    def horario_excluir(id):
        for obj in View.horario_listar():
            if obj.get_id() == id and obj.get_id_cliente() != None:
                raise ValueError("Não é possível excluir, horário já cadastrado")
    ...
    def profissional_excluir(id):
        for obj in View.horario_listar():
            if obj.get_id_profissional() == id:
                raise ValueError(
                    "Profissional com horário cadastrado, não é possível excluir"
                )
